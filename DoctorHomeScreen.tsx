...
const DoctorHomeScreen: React.FC = () => {
  ...
  const handleLinkPatient = async () => {
    ...
    try {
      ...
      if (alreadyLinked) {
        Alert.alert('Info', `Patient "${patient.fullName}" is already assigned to you.`);
        setLinking(false);
        return;
      }

      if (user && user.id) {
        await linkPatientToDoctor(patient.id, user.id);
      }
      
      await loadLinkedPatients();
      
      setPatientCode('');
      Alert.alert('Success', `Patient "${patient.fullName}" has been added to your list!`);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            <Text style={styles.statValue}>{linkedPatients.length}</Text>
        ...
        <View style={styles.linkSection}>
          <View style={styles.linkSectionHeader}>
            <View style={[styles.linkIconContainer, { backgroundColor: '#7B1FA2' }]}>
              <Text style={styles.linkIcon}>+</Text>
            </View>
            <Text style={styles.linkSectionTitle}>Add New Patient</Text>
          </View>
          
          <Text style={styles.linkSectionDesc}>
            Enter the patient's unique code to add them to your patient list
          </Text>
          ...
          {linkedPatients.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>
                Enter a patient's unique code above to add them
              </Text>
            </View>
          ) : (
