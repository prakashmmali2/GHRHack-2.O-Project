...
type DoctorRegistrationScreenProps = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'DoctorRegistration'>;
};
...
const DoctorRegistrationScreen: React.FC<DoctorRegistrationScreenProps> = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    console.log('Doctor Register clicked!');
    ...
    setIsLoading(true);
    try {
      // Check if username is available (Doctors can share usernames with other Doctors/Caregivers)
      const available = await isUsernameAvailable(username, 'Doctor');
      if (!available) {
      ...
      // Generate unique code for doctor (so patients can connect with them)
      const doctorCode = 'DR-' + generateUniqueCode().substring(3);
      ...
      if (patient && patient.id) {
        await createRelationship(patient.id, undefined, userId);
      }
      ...
      await setUser({
        ...
        fullName: name,
        role: 'Doctor',
        uniqueCode: doctorCode,
        specialization,
        createdAt: new Date().toISOString()
      });
      
      Alert.alert('Registration Successful!', `Your unique code is: ${doctorCode}\n\nShare this code with your patients so they can connect with you.`);

      console.log('Doctor registered, navigating...');
      
      // Navigate using replace to avoid back button
      navigation.replace('DoctorStack');
          ...
          <View style={styles.form}>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                autoCorrect={false}
              />
              <Text style={styles.helperText}>You can share usernames with other doctors/caregivers</Text>
            </View>
            ...
            <View style={styles.inputContainer}>
              ...
              >
                <Text style={specialization ? styles.selectText : styles.placeholderText}>
              ...
              {showSpecs && (
                <View style={styles.optionsContainer}>
                  {SPECIALIZATIONS.map((spec) => (
                    ...
                    >
                      <Text style={styles.optionText}>{spec}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
